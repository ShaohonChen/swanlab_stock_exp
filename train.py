...
def visualize_predictions(
    train_predictions, val_predictions, scaler, y_train, y_test, lookback
):
    train_predictions = inverse_transform_and_extract(
        scaler, train_predictions, lookback
    )
    ...
    plt_image.append(
        plot_predictions(
            new_y_train,
            train_predictions,
            "(TrainSet) Google Stock Price Prediction with LSTM",
        )
    )
    plt_image.append(
        plot_predictions(
            new_y_test,
            val_predictions,
            "(TestSet) Google Stock Price Prediction with LSTM",
        )
    )
...
if __name__ == "__main__":
    ...
    # ------------------- 定义数据集 -------------------
    scaler, X_train, X_test, y_train, y_test = get_dataset(
        file_path="./GOOG.csv",
    ...
    with torch.no_grad():
        ...
        visualize_predictions(
            train_predictions, val_predictions, scaler, y_train, y_test, config.lookback
        )
